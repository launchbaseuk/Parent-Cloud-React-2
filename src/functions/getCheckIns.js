export default async function getCheckIns(email, token) {
    let response = await fetch("https://hub.parent-cloud.com/wp-json/mo/v1/posts/mood_tracker", {
        headers: {
    ...
    for (let i = 0; i < response.length; i++) {
        let responseItems = await fetch(`https://hub.parent-cloud.com/wp-json/wp/v2/mood_tracker/${response[i].ID}`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${token}`
            }
