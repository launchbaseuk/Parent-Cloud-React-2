export default async function uploadCheckIn(feeling, feeling_at_the_moment, what_have_you_been_doing, cognitive_distortions, challenge, interpretation, feeling_now, email, token) {
    ...
    let responseCreate = await fetch("https://parentcloud.borne.io/wp-json/wp/v2/mood_tracker", {
        method: "POST",
        headers: {
    ...
    let responseAddInformation = await fetch(`https://parentcloud.borne.io/wp-json/acf/v3/mood_tracker/${responseCreate.id}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
