...
async function getGuides() {
  ...
  for (let i = 0; i < filters.length; i++) {
    let response = await fetch(
      `https://hub.parent-cloud.com/wp-json/wp/v2/guides?master_filter=${filters[i].id}&per_page=100`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function getVideos() {
  ...
  for (let i = 0; i < filters.length; i++) {
    let response = await fetch(
      `https://hub.parent-cloud.com/wp-json/wp/v2/videos?master_filter=${filters[i].id}&per_page=100`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function getPodcasts() {
  ...
  let podcasts = [];
  const response = await fetch(
    `https://hub.parent-cloud.com/wp-json/wp/v2/videos?per_page=100`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
async function getGuidedMeditation() {
  ...
  let response = await fetch(
    `https://hub.parent-cloud.com/wp-json/wp/v2/guided_meditation?per_page=100`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
async function getSleepStories() {
  ...
  let response = await fetch(
    `https://hub.parent-cloud.com/wp-json/wp/v2/sleep_stories?per_page=100`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
