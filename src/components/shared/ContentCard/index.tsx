...
export default function ContentCard({image, text, route}: any) {
  const onPress = async () => {
    if (route == 'PocketCBT') {
      if (
        (await AsyncStorage.getItem('pocketCBT')) ||
        (await AsyncStorage.getItem('pocketCBT')) == 'true' ||
        !((await AsyncStorage.getItem('pocketCBT')) == null) ||
        (await AsyncStorage.getItem('pocketCBT')) == 'false'
      ) {
