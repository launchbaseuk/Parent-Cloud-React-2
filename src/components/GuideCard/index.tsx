    ...
    <View style={styles.guidecardsmallContainer}>
      ...
      <View
        ...
        <PrimaryButton
          ...
          text="Download"
          onPress={() =>
            navigation.navigate('PDFViewer', {title: decode(title), text: decode(redirect), pdf: fileLink})
          }
        />
      </View>
    </View>
...
function GuideCardBig({title, excerpt, content, fileLink, bookmarked, onPressBookmark, postid}: any) {
  ...
  useEffect(() => {
    (async() => {
      ...
      const requestTwo = await fetch(`https://hub.parent-cloud.com/wp-json/wp/v2/media/${response.featured_media}`);
      ...
      <View
        ...
        <View style={{alignSelf: 'flex-end', marginBottom: 6}}>
          <PrimaryButton
            size="mini"
            text="Download"
            onPress={() => navigation.navigate('PDFViewer', {title: decode(title), text: decode(guide.content.rendered), pdf: fileLink})}
          />
        </View>
      </View>
    </View>
